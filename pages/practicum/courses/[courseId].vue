  ...
  <ClientOnly>
    <PracticumShell :context-title="plan?.title ?? '课程详情'" context-meta="课程介绍、训练结构与学习入口">
      ...
      <section v-else class="course-detail" data-course-detail>
        ...
        <header class="course-detail-hero"><div><p class="eyebrow">{{ plan.status === 'PUBLISHED' ? '进行中课程' : '课程计划' }}</p><h1>{{ plan.title }}</h1><p>{{ plan.description || '该课程将用真实业务任务串联学习、实操、提交和反馈。' }}</p></div><NuxtLink v-if="store.state.activeRole === 'STUDENT'" :to="`/practicum/learn/${plan.id}`" class="primary-button">进入学习</NuxtLink><NuxtLink v-else-if="canEditPlan(store.state.activeRole)" :to="`/practicum/plans/${plan.id}/edit`" class="primary-button">管理课程</NuxtLink></header>
...
const plan = computed(() => backendDetail.value?.plan ?? store.state.plans.find(item => item.id === route.params.courseId))
const detailNodes = computed(() => backendDetail.value?.nodes ?? (plan.value ? store.getPlanNodes(plan.value.id) : []))
