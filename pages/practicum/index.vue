...
const serverPlans = ref<Plan[]>([])
const serverPlansLoaded = ref(false)
const plans = computed(() => serverPlansLoaded.value && serverPlans.value.length
  ? serverPlans.value
  : store.visiblePlansFor(store.state.activeRole))
...
const firstPublishedPlan = computed(() => plans.value.find(plan => plan.status === 'PUBLISHED' && !isShellCourse(plan)) ?? null)
...
onMounted(async () => {
  ...
  try {
    const response = await server.listPlans({
      status: user?.role === 'STUDENT' ? 'PUBLISHED' : undefined,
      page: 1,
      pageSize: 50,
      sort: 'updatedAt',
      direction: 'desc',
