...
import { updatePlan } from '../../../utils/practicum-repository'

export default defineEventHandler(async (event) => {
  const result = updatePlan(requireAuthenticatedUser(event), getRouterParam(event, 'planId') ?? '', await readBody(event))
