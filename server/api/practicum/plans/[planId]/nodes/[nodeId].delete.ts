...
import { deleteCurriculumNode } from '../../../../../utils/practicum-repository'

export default defineEventHandler(async (event) => {
  const result = deleteCurriculumNode(requireAuthenticatedUser(event), getRouterParam(event, 'planId') ?? '', getRouterParam(event, 'nodeId') ?? '', await readBody(event))
