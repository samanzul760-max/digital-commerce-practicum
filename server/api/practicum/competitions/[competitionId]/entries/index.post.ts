...
export default defineEventHandler(async (event) => {
  const body = await readBody<{ statement?: string }>(event)
  const result = enterCompetition(requireAuthenticatedUser(event), getRouterParam(event, 'competitionId') ?? '', body?.statement?.trim() ?? '')
