...
test.describe('platform API contract', () => {
  test('owner resource list supports search and CRUD', async ({ page }) => {
    ...
    const created = await page.request.post('/api/practicum/resources', { headers: await csrfHeaders(page, { 'Idempotency-Key': key }), data: { planId: 'library', name: `资料 ${key}`, kind: 'LINK', url: 'https://example.test/resource' } })
    ...
    const removed = await page.request.delete(`/api/practicum/resources/${resource.id}`, { headers: await csrfHeaders(page) })
  ...
  test('asset upload enforces type and size policy', async ({ page }) => {
    ...
    const bad = await page.request.post('/api/practicum/assets', { headers: await csrfHeaders(page), multipart: { file: { name: 'script.exe', mimeType: 'application/x-msdownload', buffer: Buffer.from('not allowed') } } })
  ...
  test('stale plan version is rejected without overwriting newer data', async ({ page }) => {
    ...
    const created = await page.request.post('/api/practicum/plans', { headers: await csrfHeaders(page, { 'Idempotency-Key': key }), data: { title: `冲突测试 ${key}`, description: '版本校验', roomId: 'room-001' } })
    const plan = (await created.json()).plan
    await page.request.patch(`/api/practicum/plans/${plan.id}`, { headers: await csrfHeaders(page), data: { description: '第一次更新', version: plan.version } })
    const stale = await page.request.patch(`/api/practicum/plans/${plan.id}`, { headers: await csrfHeaders(page), data: { description: '覆盖尝试', version: plan.version } })
