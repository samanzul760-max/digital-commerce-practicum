...
test('[CASE-S2-001] administrator creates a draft plan hidden from students with editor entry', async ({ page }) => {
  ...
  await expect(createdPlan.locator('a')).toHaveAttribute('href', /\/practicum\/plans\/plan-custom-\d+\/edit$/)
  await createdPlan.locator('a').click()
  await expect(page).toHaveURL(/\/practicum\/plans\/plan-custom-\d+\/edit$/)
